...
import rocks.shumyk.photoapp.api.albums.ui.model.AlbumResponseModel;
...
public class AlbumsController {
    ...
    public List<AlbumResponseModel> userAlbums(@PathVariable final String userId) {
        final List<AlbumEntity> albumEntities = albumsService.getAlbums(userId);
        log.info("Returning {} albums", albumEntities.size());
        return mapper.map(albumEntities, new TypeToken<List<AlbumResponseModel>>(){}.getType());
    }
}
